  ...
  create or replace view `genai-copilot-enterprisedata`.`staging`.`stg_orders`
  OPTIONS()
  as -- stg_orders: clean column names, cast types, filter bad rows
-- Raw → Staging: rename, cast, no business logic yet

with source as (
    select * from `genai-copilot-enterprisedata`.`raw`.`orders`
),
