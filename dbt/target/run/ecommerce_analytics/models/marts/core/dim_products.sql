    ...
    create or replace table `genai-copilot-enterprisedata`.`marts`.`dim_products`
...
final as (
    select
        -- ── Key ─────────────────────────────────────────────────────────────
        to_hex(md5(cast(coalesce(cast(p.product_id as string), '_dbt_utils_surrogate_key_null_') as string)))
                                                        as product_sk,
        p.product_id,
